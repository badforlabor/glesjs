...
void __bufferData(const v8::FunctionCallbackInfo<v8::Value> &args)
{
	HandleScope handle_scope(js->isolate);
	unsigned int target = (unsigned int)args[0]->IntegerValue();
	v8::Handle<v8::ArrayBufferView> arrayBufferView = Handle<ArrayBufferView>::Cast(args[1]);
	Handle<ArrayBuffer> buf_data = arrayBufferView->Buffer();
	...
	LOGI("Data sz=%d coord 1:%f,%f", (int)size, data[4], data[5]);
	float data2[] = {
		1,
		4,
		3,
