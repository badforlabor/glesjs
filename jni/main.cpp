				...
				int type)
{
	...
	else if (args[dataidx]->IsArray())
	{
		...
		{ // UNIFORMFLOAT, UNIFORMMATRIXFLOAT
...
void MyJS::callFunction(const char *funcname, const int argc, Handle<Value> argv[])
{
	// init
	Isolate::Scope isolate_scope(isolate);
	HandleScope handle_scope(isolate);
...
{
	LOGI("js 1");
	LOGI("js-version:%s", v8::V8::GetVersion());
	js = new MyJS();
	LOGI("js 2");
	//V8::SetArrayBufferAllocator(new MallocArrayBufferAllocator());
}
 ...
 * Signature: (II)V
 */
	JNIEXPORT void JNICALL Java_net_tmtg_glesjs_GlesJSLib_onSurfaceChanged(JNIEnv *env, jclass clas, jint width, jint height)
	{
		...
		if (!app_was_inited)
		{
			init_javascript();
			LOGI("2");
			for (int i = 0; i < NR_PLAYERS * PLAYERDATASIZE; i++)
				gamepadvalues[i] = 0;
		}
		LOGI("3");
		screenwidth = width;
 ...
 * Signature: ()V
 */
	JNIEXPORT void JNICALL Java_net_tmtg_glesjs_GlesJSLib_onDrawFrame(JNIEnv *env, jclass clas)
	{
 ...
 */
	...
	JNIEXPORT void JNICALL Java_net_tmtg_glesjs_GlesJSLib_onTouchEvent(JNIEnv *env, jclass clas, jint ptrid, jdouble x, jdouble y,
 ...
 * Signature: (IDD)V
 */
	JNIEXPORT void JNICALL Java_net_tmtg_glesjs_GlesJSLib_onMultitouchCoordinates(JNIEnv *env, jclass clas, jint ptrid, jdouble x, jdouble y)
	{
 ...
 * Signature: (IZ[I[F)V
 */
	JNIEXPORT void JNICALL Java_net_tmtg_glesjs_GlesJSLib_onControllerEvent(JNIEnv *env, jclass clas, jint player, jboolean active,
